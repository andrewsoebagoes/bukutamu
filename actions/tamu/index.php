    ...
    foreach($data as $key => $d)
    {
        ...
        foreach($columns as $col)
        {
            ...
            if(is_array($field))
            {
                ...
                // if($field['label'] == 'Tanggal' && $data_value){
                //     $data_value = date("d-m-y", $d->created_at);
                // }

                if($field['label'] == 'Ananda' && $data_value){
                    $data_value = unserialize($data_value);
                    $data_value = array_map(function($v){
                        return "$v[nama_ananda],$v[tahun_lahir_ananda],$v[status_ananda]";
                    }, $data_value);
                    $data_value = implode("<br>", $data_value );
                }
            }
            else
            {
